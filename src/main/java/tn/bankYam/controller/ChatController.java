...
public class ChatController {
	...
	@GetMapping("room")
	private String chatRoom(long cr_seq){
		return "chatRoom";
	}

	@GetMapping("insert")
	private String insertRoom(@RequestParam ArrayList<Long> f_mb_seq, HttpSession session, HttpServletResponse response) throws IOException{
		System.out.println(f_mb_seq);
		Membery membery = (Membery) session.getAttribute("membery");
		if(f_mb_seq.size() == 1){
			Chatmember chatmember= chatroomService.checkRoomS(membery.getMb_seq(), f_mb_seq.get(0));
			if(chatmember != null){
				ScriptUtil.newWindow(response, "room?cr_seq="+chatmember.getCm_cr_seq());
